...
app.get('/api/publicacoes', (req, res) => {
    const query = 'SELECT id_publicacao, foto, nome_post, local, cidade, usuarioid FROM publicacao';
    connection.query(query, (err, results) => {
...
app.get('/api/minhas-publicacoes/:usuarioId', (req, res) => {
    const { usuarioId } = req.params;
    const query = 'SELECT id_publicacao, foto, nome_post, local, cidade, usuarioid FROM publicacao WHERE usuarioid = ?';
    connection.query(query, [usuarioId], (err, results) => {
...
// Cadastrar post
app.post('/cadastrar-post', upload.single('foto'), (req, res) => {
    const { nome_post, local, cidade, usuarioid } = req.body;
    const foto = req.file ? `/images/${req.file.filename}` : null;

    console.log('Dados recebidos:', { nome_post, local, cidade, usuarioid, foto });

    if (!foto || !nome_post || !local || !cidade || !usuarioid) {
        return res.status(400).json({ success: false, message: 'Todos os campos são obrigatórios.' });
    }

    const query = 'INSERT INTO publicacao (foto, nome_post, local, cidade, usuarioid) VALUES (?, ?, ?, ?, ?)';
    connection.query(query, [foto, nome_post, local, cidade, usuarioid], (err, results) => {
        if (err) {
            console.error('Erro ao cadastrar post:', err);
            return res.status(500).json({ success: false, message: 'Erro ao cadastrar post.', error: err.message });
        }
        res.json({ success: true, message: 'Post cadastrado com sucesso!', publicacaoId: results.insertId });
    });
});

// Editar publicação
app.put('/editar-publicacao', upload.single('foto'), (req, res) => {
    const { nome_post, local, cidade, publicacaoId, usuarioId } = req.body;
    const foto = req.file ? `/images/${req.file.filename}` : null;

    if (!nome_post || !local || !cidade || !publicacaoId || !usuarioId) {
        return res.status(400).json({ success: false, message: 'Dados obrigatórios ausentes.' });
    }
    ...
    connection.query(selectQuery, [publicacaoId, usuarioId], (err, results) => {
        ...
        const updateQuery = `
            UPDATE publicacao 
            SET nome_post = ?, local = ?, cidade = ?, foto = ? 
            WHERE id_publicacao = ? AND usuarioid = ?
        `;
        connection.query(updateQuery, [nome_post, local, cidade, foto || currentFoto, publicacaoId, usuarioId], (err, results) => {
